...
//     experiments/ff1_w4a4_swiglu_topk_reduce_mech_actreg_n256pf.cu -o /tmp/ff1_w4a4_actreg_n256pf
//
...
#if N256PF
#define MODE_NAME "no_workspace_n16x2_n256pf4x32_tokenrow32_scatter_ca_scalar"
#else
#define MODE_NAME "no_workspace_n16x2_no_pf_tokenrow32_scatter_ca_scalar"
#endif
...
/*
    ...
    X4[token,kt,p] u32
      one token / K64 = 4 p lanes x 8B = one contiguous 32B packet
      adjacent routed tokens are H64*32B apart at fixed kt

    Sx[token,kt] u32
      one token / K64 = one UE4M3x4 word = 4B

    n256 producer, independent from later N16 consumer ownership:
      q=t&255, band=t>>8, token=(n256<<8)+q
      each live q hints X4[kt4*4+band,token], 32B
      four 256T bands cover four K64 leads; no PF wait
...
void ff1_w4a4_swiglu_topk_reduce_mech_actreg_n256pf(
...
#if N256PF
    // n256 PF ownership is independent from the N16 MMA consumer below.
    // b0: t000..255, b1: t256..511, b2: t512..767, b3: t768..1023
    // every band keeps q=t&255, tests the same routed token, and owns one K64.
    for (int n256 = 0; n256 < (((uint32_t)NP + 255u) >> 8); n256++) {
        live16 = 0u;
        if (((uint32_t)(n256 << 3)
             + (((uint32_t)threadIdx.x & 255u) >> 5))
            < (uint32_t)(Xb_stride - 1)) {
            asm volatile(
                "ldu.global.u32 %0, [%1];"
                : "=r"(live16)
                : "l"((uint64_t)__cvta_generic_to_global(
                      Xb + (uint64_t)blockIdx.x * (uint32_t)Xb_stride + 1u
                      + (uint32_t)(n256 << 3)
                      + (((uint32_t)threadIdx.x & 255u) >> 5)))
                : "memory");
        }
        live16 = (live16 >> ((uint32_t)threadIdx.x & 31u)) & 1u;
        if ((((uint32_t)n256 << 8) + ((uint32_t)threadIdx.x & 255u))
                < (uint32_t)NP
            && live16) {
            for (int kt4 = 0; kt4 < (((uint32_t)H >> 6) + 3u) >> 2; kt4++) {
                if (((uint32_t)(kt4 << 2) + ((uint32_t)threadIdx.x >> 8))
                    < ((uint32_t)H >> 6)) {
                    asm volatile(
                        ...
                        :
                        : "l"((uint64_t)__cvta_generic_to_global(
                              reinterpret_cast<const char*>(X4)
                              + ((uint64_t)((uint32_t)(kt4 << 2)
                                  + ((uint32_t)threadIdx.x >> 8))
                                  * (uint32_t)NP
                                  + ((uint32_t)n256 << 8)
                                  + ((uint32_t)threadIdx.x & 255u)) * 32u))
                        : "memory");
                }
            }
        }
...
#else
                    // The route-scattered n256 producer ran before N16 scan.
                    // Consumer ownership starts here; no hint is replayed by i/plane.
#endif
                        ...
                        if (live16b) {
                            ...
                            if (xbm3) {
                                asm volatile(
                                    ...
                                    : "l"(X4
                                        + (((uint64_t)((hp << 5) + k)
                                            * (uint32_t)NP + ((uint32_t)n16b << 4) + 8u
                                            + (((uint32_t)threadIdx.x & 31u) >> 2)) << 3)
                                        + (((uint32_t)threadIdx.x & 3u) << 1))
                                    : "memory");
                            }
...
__global__ void init_X4(uint32_t* p, uint64_t n, uint32_t NP) {
    uint64_t x = (uint64_t)blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t stride = (uint64_t)blockDim.x * gridDim.x;
    for (; x < n; x += stride) {
        uint32_t token = (uint32_t)((x >> 3) % NP);
...
int main(int argc, char** argv) {
    ...
    uint64_t pf_bytes = N256PF ? assignments * H64 * 32u : 0u;
    ...
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &blocks_per_sm, ff1_w4a4_swiglu_topk_reduce_mech_actreg_n256pf,
        CTA, smem_bytes));
    ...
    std::fprintf(f,
        "%s,%d," MODE_NAME ",balanced_exact_live_scattered,"
