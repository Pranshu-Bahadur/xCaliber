...
//     experiments/ff1_w4a4_swiglu_topk_reduce_mech_actreg_n8h128.cu -o /tmp/ff1_w4a4_actreg_n8h128
//
...
/*
    ...
    X4[kt,n8,lane,{lo,hi}] u32
      one K64 / N8 = 32 lanes x 8B = 256B
      two adjacent K64 rows fill {b00,b01,b10,b11}

    Sx[kt,n8,g] u32
      one K64 / N8 = 8 token rows x one UE4M3x4 word = 32B

    Four 256T prefetch bands each own eight K64 rows per H2048 panel.
    Every N8 sector is hinted once; matching warp loads have no PF wait.

    routed N8 x H128
      exact live N8 scan, no dead partner MMA
      MMA0 consumes K64[2kh], MMA1 consumes K64[2kh+1]
      2x N iterations, 1/2 H iterations, same useful MMA count

    dynamic SMEM
      CTA*4B fragment-native BF16 output only
...
void ff1_w4a4_swiglu_topk_reduce_mech_actreg_n8h128(
        ...
        for (int i = 0; i < (I >> 10); i++) {
            for (int plane = 0; plane < 4; plane++) {
                ...
                for (int hp = 0; hp < ((H + 2047) >> 11); hp++) {
                    // Four 256T bands: 8 K64 rows x 8 X4 sectors per band.
                    if (((uint32_t)threadIdx.x & 255u) < 64u
                        && ((hp << 5)
                            + (((uint32_t)threadIdx.x >> 8) << 3)
                            + (((uint32_t)threadIdx.x & 255u) >> 3)
                            < ((uint32_t)H >> 6))) {
                        asm volatile(
                            ...
                            : "l"((uint64_t)__cvta_generic_to_global(
                                  ...
                                  + ((uint64_t)((hp << 5)
                                      + (((uint32_t)threadIdx.x >> 8) << 3)
                                      + (((uint32_t)threadIdx.x & 255u) >> 3))
                                      * ((uint32_t)NP >> 3) + (uint32_t)n8) * 64u
                                  + (((uint32_t)threadIdx.x & 7u) << 3)))
                            : "memory");
                    }
                    ...
                    for (int kh = 0;
                        ...
                        if (!((uint32_t)threadIdx.x & 3u) && xbm0) {
                            asm volatile(
                ...
                asm volatile("cvt.rn.bf16x2.f32 %0, %1, %2;"
                             : "=r"(out) : "f"(o1), "f"(o0));
                reinterpret_cast<uint32_t*>(smem + ACT_OUT_OFFSET)[threadIdx.x] = out;
...
__global__ void init_X4(uint32_t* p, uint64_t n, uint32_t N8) {
    uint64_t x = (uint64_t)blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t stride = (uint64_t)blockDim.x * gridDim.x;
    for (; x < n; x += stride) {
        uint32_t token = (uint32_t)((x >> 6) % N8) * 8u
                       + (((uint32_t)x >> 1) & 31u) / 4u;
...
int main(int argc, char** argv) {
    ...
    uint64_t mma_inst = active_n8_tiles * (uint64_t)(I >> 10) * 4u
                      * (uint64_t)(H >> 6) * 32u;
    ...
    uint64_t pf_bytes = active_n8_tiles * activation_replay * H64 * 288u;
    uint64_t W_bytes = active_n8_tiles * activation_replay * H64 * 16384u;
    uint64_t S_bytes = active_n8_tiles * activation_replay * H64 * 2048u;
    uint64_t reduce_bytes = active_n8_tiles * (uint64_t)(I >> 10) * 4u * 4096u;
    int blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &blocks_per_sm, ff1_w4a4_swiglu_topk_reduce_mech_actreg_n8h128, CTA, smem_bytes));
    ...
    std::fprintf(f,
        "%s,%d,no_workspace_n8_h128_pf32_actreg_ca_scalar,balanced_exact_live_scattered,"
        ...
        live_experts ? (double)active_n8_tiles / live_experts : 0.0,
