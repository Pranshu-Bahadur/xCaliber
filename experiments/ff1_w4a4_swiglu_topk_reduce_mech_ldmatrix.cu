...
//     experiments/ff1_w4a4_swiglu_topk_reduce_mech_ldmatrix.cu -o /tmp/ff1_w4a4_ldmatrix
//
...
/*
    ...
    X4[kt,n16,matrix4,row8,col8] b16
      m0/m1 = N0..7  x K0..31/K32..63
      m2/m3 = N8..15 x K0..31/K32..63
      one ldmatrix.x4.trans gives {b00,b01,b10,b11}
...
void ff1_w4a4_swiglu_topk_reduce_mech_ldmatrix(
    ...
    uint32_t live16, live16b, epoch = 0, ready_epoch0 = 0, ready_epoch1 = 0;
            ...
            for (int plane = 0; plane < 4; plane++) {
                ...
                if (live16 & 0xffu) {
                    ...
                    asm volatile("cvt.rn.bf16x2.f32 %0, %1, %2;"
                                 : "=r"(out) : "f"(o1), "f"(o0));
                    reinterpret_cast<uint32_t*>(smem + (stage_bytes << 1))
                        [threadIdx.x] = out;
...
__global__ void init_X4(uint32_t* p, uint64_t n, uint32_t N16) {
    ...
    for (; x < n; x += stride) {
        uint32_t token = (uint32_t)((x >> 7) % N16) * 16u
                       + ((((uint32_t)x >> 5) & 3u) >> 1) * 8u
                       + (((uint32_t)x & 3u) << 1);
        uint32_t q = 1u + ((token + (token >> 3)) & 3u);
        token++;
        p[x] = q * 0x1111u
             | (1u + ((token + (token >> 3)) & 3u)) * 0x11110000u;
    }
}
...
int main(int argc, char** argv) {
    ...
    int smem_bytes = stage_k64 * 2304 + CTA * 4;
    ...
    uint64_t X4_bytes = active_route_tiles * activation_replay * H64 * 512u;
    uint64_t Sx_bytes = active_route_tiles * activation_replay * H64 * 64u;
    ...
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &blocks_per_sm, ff1_w4a4_swiglu_topk_reduce_mech_ldmatrix, CTA, smem_bytes));
    ...
    std::fprintf(f,
        "%s,%d,no_workspace_n16x2_pf32_tma_ldmatrix_x4,balanced_exact_live_scattered,"
        ...
        (unsigned long long)(X4_bytes + Sx_bytes),
