// ZeroWire: SM120a W4A4 FF1 -> SwiGLU -> topk_W -> BF16 bulk-reduce.
//
// Build:
//   nvcc -O3 -std=c++17 -arch=sm_120a -lineinfo -Xptxas=-v \
//     experiments/x57.cu -o /tmp/x57
//
...
/*
    ...
    W13[e,kt,i1024,plane4,t1024,v4]
      ...
      no sign-bit transform exists in ZeroWire
...
void x57(
    ...
    uint32_t live16, live16b, route_word;
#if !ACTREG
...
#define ACT_OUT_OFFSET (stage_bytes << 1)
    ...
    for (int n256 = 0; n256 < ((NP + 255) >> 8); n256++) {
        route_word = 0u;
        if (((uint32_t)threadIdx.x & 31u) < 8u
            && (n256 << 3) + ((uint32_t)threadIdx.x & 31u) < Xb_stride - 1) {
            asm volatile("ldu.global.u32 %0, [%1];"
                         : "=r"(route_word)
                         : "l"((uint64_t)__cvta_generic_to_global(
                               Xb + (uint64_t)blockIdx.x * (uint32_t)Xb_stride
        ...
        for (int n16 = n256 << 4;
             n16 < (NP >> 4) && n16 < ((n256 + 1) << 4); n16++) {
        live16 = __shfl_sync(
            0xffffffffu, route_word, ((uint32_t)n16 & 15u) >> 1);
        live16 = (live16 >> ((n16 & 1) << 4)) & 0xffffu;
        if (!live16) continue;
        live16b = 0u;
        for (n16b = n16 + 1;
             n16b < (NP >> 4) && n16b < ((n256 + 1) << 4) && !live16b;
             n16b += !live16b) {
            live16b = __shfl_sync(
                0xffffffffu, route_word, ((uint32_t)n16b & 15u) >> 1);
            live16b = (live16b >> ((n16b & 1) << 4)) & 0xffffu;
        }
        xbm0 = live16 & XbLUT[((uint32_t)threadIdx.x & 31u) >> 2];
        xbm1 = live16 & XbLUT[8u + (((uint32_t)threadIdx.x & 31u) >> 2)];
        xbm2 = live16b & XbLUT[((uint32_t)threadIdx.x & 31u) >> 2];
        xbm3 = live16b & XbLUT[8u + (((uint32_t)threadIdx.x & 31u) >> 2)];
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMemcpy(dTopk, hTopk.data(), hTopk.size() * sizeof(__nv_bfloat16),
                          cudaMemcpyHostToDevice));
    init_W13_preflipped<<<4096, 256>>>(dW, W_u32);
    ...
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &blocks_per_sm, x57, CTA, smem_bytes));
    ...
    std::fprintf(f,
        "%s,%d,x57,balanced_exact_live_scattered,"
