...
//     experiments/ff1_w4a4_swiglu_topk_reduce.cu -o /tmp/ff1_w4a4
//
...
void ff1_w4a4_swiglu_topk_reduce(
            ...
            for (int plane = 0; plane < 4; plane++) {
                ...
                for (int hp = 0; hp < ((H + 2047) >> 11); hp++) {
                    ...
                    for (int k = 0; k < 32 && ((hp << 5) + k) < (H >> 6); k++) {
                        ...
                        // Native e2m1 sign bit. Both FF1 branches are negated;
                        // their signs cancel in the SwiGLU elementwise product.
                        a0 ^= 0x88888888u;
                        a1 ^= 0x88888888u;
                        a2 ^= 0x88888888u;
                        a3 ^= 0x88888888u;
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &blocks_per_sm, ff1_w4a4_swiglu_topk_reduce, CTA, smem_bytes));
    ...
    std::fprintf(f,
        "%s,%d,no_workspace_n16x2_pf32_tma,balanced_exact_live_scattered,"
