...
CSV_PATH = Path(
    os.environ.get("CSV_PATH", "/content/xr64abs_qwen3_smoke.csv")
)
OUTPUT_PATH = Path(
    os.environ.get("OUTPUT_PATH", "/content/xr64abs_qwen3_smoke.txt")
)
...
if capability != (12, 0):
    raise RuntimeError(f"xR64ABS G4 smoke requires sm_120, got {capability}: {gpu}")
...
print(f"gpu={gpu} sm={capability[0]}{capability[1]} load_s={load_s:.3f}")
print("+--------+--------------+---------------+------------------+")
print("| trial  | generation_s | output_tokens | aggregate tok/s  |")
print("+--------+--------------+---------------+------------------+")
for row in rows:
